-- ServerScriptService/atonement_r6.lua
...
-- Place this script in ServerScriptService. If you already have an Atonement Tool, the
-- script will find it by name in ReplicatedStorage, ServerStorage, StarterPack or workspace.
...
local function onToolFound(tool)
	...
	local function onEquipped(playerTool)
		...
		local function onActivated()
			...
			pos = pos or (player and player:GetMouse().Hit.p) or workspace:GetServerTimeNow() and workspace:GetServerTimeNow() and workspace:FindFirstChildWhichIsA("BasePart") and workspace:FindFirstChildWhichIsA("BasePart").Position or Vector3.new(0,0,0)
			-- find nearest R6 NPC
			...
			playAnimationOnModel(target, animId)
		end
		...
		local function connectToolInstance()
			...
			if tinst and tinst:IsA("Tool") then
				-- connect Activated
				if not tinst:IsDescendantOf(player.Character) then
					onActivated = onActivated -- reuse closure
				end
